...
Do{
    ...
    Switch (Show-Menu $menu "London Dealing Room Support" -clear) {
        ...
        "2" {Import-Module activedirectory
                ...
                {Write-Output $strComputer $username " | Description: " $description.description -ForegroundColor Green}
                Else
                {$description = Get-ADComputer $strComputer -Properties description
                Write-Output "Nobody logged in on $StrComputer | Description: " $description.description -ForegroundColor Red} 
             sleep -seconds 5
        ...
        "3" {Import-Module activedirectory
                ...
                Write-Output "Your user information is now on your screen" -ForegroundColor Green
              }
        ...
        "4" { $ComputerName='LOTD122159'
                ...
                $regkey = $reg.OpenSubKey("SOFTWARE\\Policies\\Microsoft\\Windows NT\\Terminal Services",$true)
                $regkey.SetValue('fDenyTSConnections','0','DWord')   
                Write-Output "Enabling Remote Desktop on $Computername...Complete"
                start-process "C:\Windows\System32\mstsc.exe" -argumentlist "/v:$Computername /f"
                }
        ...
        "5"    {$Pcname = Read-host -prompt "Please enter the hostname"
            ...
            {Write-Output "Incorrect security code" -ForegroundColor Red}
            else
            {
            (Get-WmiObject -Class Win32_operatingsystem -ComputerName $Pcname).win32shutdown(4)
                    
            Write-Output "Everyone will be logged off" -ForegroundColor Green
            }
        ...
        "7"  {$ComputerName = Read-Host -prompt "Enter hostname" 
            ...
            Write-Output "Enter hostname $Computername"

            $reg = [Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey('LocalMachine', $ComputerName) 
            $regkey = $reg.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\StorageDevicePolicies",$true)
            $regkey.SetValue('WriteProtect','0','DWord')  
		
            Write-Output "Value in the registry has changed: $ComputerName" -ForegroundColor Green
            sleep -seconds 5
         }

        "8"  {$path = read-host -prompt "Enter Path of file/folder"
                $destination = read-host -prompt "Enter path destination"
                Write-Output "Enter Path of file/folder $path"
                Write-Output "Enter path destination $destination"
                Copy-Item -Path $path -Destination $destination
                Write-Output "file/folder has been copied" -ForegroundColor Green
                sleep -Seconds 3
               }
     ...
     "12" {       
            ...
            Catch
            {
                Write-Output "$PCName not in AD" -ForegroundColor Red   
                sleep -seconds 3
            }
         }
    ...
    "13" {
        ...
        if($PCPrefix -ne "")
        {
            ...
            {
                Write-Output "Prefix must be: LOND, LOTD, CDCD, UTTD or UTCD" -ForegroundColor red
                sleep -seconds 3
                $computers = $null
            }
        ...
        {
            Write-Output "All london computers"
        ...
        ForEach($Computer in $Computers)
        {
            ...
            if($User -ne $Null)
            {
                if($User -eq $FindUser)
                {
                    Write-Output "$User is logged in to $Computer"  -ForegroundColor Green
                    sleep -seconds 3
    ...
    "15" {
            ...
            {
                Write-Output "File: C:\Temp\GroupMembers.csv is slready open - close and rerun" -ForegroundColor Red
                sleep -seconds 3
            }
            else
            {
               ...
               ELSE
                {
                    Write-Output "Group: $Group not found in AD" -ForegroundColor Red
                    sleep -seconds 3
                }
        }
    }
    ...
    "16" {
            ...
            {
                Write-Output "File: C:\Temp\UserGroups.csv is slready open - close and rerun" -ForegroundColor Red
                sleep -seconds 3
            }
            else
            {
               $User = read-host -Prompt "Enter user name"
                ...
                ELSE
                {
                    Write-Output "User: $User not found in AD" -ForegroundColor Red
                    sleep -seconds 3
                } 
            }
        }
    ...
    "17" {
            ...
            {
                Write-Output -ForegroundColor Green "User: $User is in group: $Group"
                sleep -seconds 3
            }
            else
            {
                Write-Output -ForegroundColor Red "User: $User is not in group: $Group"
                sleep -seconds 3
            }
    ...
    "18"  {
            ...
            if($ADUser -ne $Null)
            {
                ...
                If(test-path $DestinationPath)
                {
                    if(Get-WriteAccess $DestinationPath)
                    {
                                ...
                                if($User -eq $FindUser)
                                {
                                    ...
                                    if(test-path $SourcePath)
                                    {
                                        ...
                                        ForEach($LogFile in $LogFiles)
                                        {
                                            ...
                                            }#Write-Output "$LogFile ...copied to $SourcePath"
                                        }#End loop through log files
                                        Write-Output "ION logs copied for user: $User to $DestinationPath\$User"  -ForegroundColor Green
                                        sleep -seconds 3
                                        read-host -prompt "Press Enter to continue"
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            Write-Output "$SourcePath not found"
                                            sleep -Seconds 3
                                        }
                                        else
                                        {
                                            Write-Output "Error: ION not installed on $Computer"
                                            sleep -Seconds 3
                                        }
                        ...
                        if($User -eq $Null)
                        {
                            Write-Output "User: $FindUser not logged in (LOTD, LOND or CDCD)"
                            sleep -Seconds 3
                        }
                    }
                    else
                    {
                        Write-Output "You do not have write access to $DestinationPath"  
                        sleep -Seconds 3
                    }
                }
                else
            ...
            {
                Write-Output "User: $FindUser not in AD"
                sleep -Seconds 3
            }#End ADUser Null
        }
    ...
    "19" {
        ...
        {
            Write-Output "Output csv file C:\temp\Users.csv already open - close and rerun" -ForegroundColor Red
            sleep -seconds 3
        }
        Else
        {   
            if (test-path -Path "c:\temp\PCs.csv") 
            {
                ...
                Write-Output "Please wait while csv file is created..." -ForegroundColor Green
            ...
            {
                Write-Output "C:\Temp\PCs.csv not found - please create file for input" -ForegroundColor Red
                sleep -seconds 3
    ...
    "99" {# Force Reboot 
               $Hostname = Read-Host "Enter Hostname"
                        ...
                        Write-Output "$hostname has been rebooted" -ForegroundColor Green
                        Sleep -Seconds 2
                
                        
         } 

        "Q" {Write-Output "Goodbye" -ForegroundColor Green
                Return
                }
